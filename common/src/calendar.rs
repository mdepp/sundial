...
pub mod orbit {
    ...
    pub fn get_next_cardinal_date(
        ...
        let angle = get_axis_angle(instant);

        match angle {
            _ if angle <= 0.5 => ("Summer Solstice", get_time_from_angle(instant, 0.0), true),
            _ if angle <= 89.5 => ("Fall Equinox", get_time_from_angle(instant, 90.0), false),
            _ if angle <= 90.5 => ("Fall Equinox", get_time_from_angle(instant, 90.0), true),
            _ if angle <= 179.5 => (
                "Winter Solstice",
                get_time_from_angle(instant, 180.0),
                false,
            ),
